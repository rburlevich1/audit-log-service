...
## Дополнение при реализации T7

T7 содержит жёсткий merge gate: "at least one week in staging with
`EXPLAIN ANALYZE` on representative production queries showing the four
composite indexes from T2 are used; legacy single-column indexes are
not chosen for the read path." Эта проекта не имеет staging-окружения,
поэтому evidence физически собрать нельзя.

Решение: подготовить артефакты (`V3` миграция, обновлённый
`AuditEventMigrationTest`) как готовый PR, но НЕ мерджить пока gate не
выполнен или явно отозван. PR открыт как draft и описание явно говорит,
что merge заблокирован отсутствующим staging evidence.

Если staging gate невыполним в принципе (homework/learning project,
production не существует), команде нужно явно решить:

1. Признать gate невыполнимым и переписать `tasks.md` § T7 на
   "local Testcontainers verification only" — затем замерджить PR.
2. Оставить PR в draft до появления staging.
3. Закрыть PR и удалить T7 из roadmap.

Куда внесено:

- `src/main/resources/db/migration/V3__drop_legacy_single_column_indexes.sql`
  создан с `DROP INDEX IF EXISTS` для трёх legacy индексов.
- `AuditEventMigrationTest.migrationsLeaveOnlyCompositeQueryApiIndexes`
  обновлён: ожидает только четыре composite индекса, утверждает что
